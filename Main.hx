...
class Main {
     ...
     * 开始一场AI自动对战
     */
    public static function startAIBattle():Void {
        if (ai.AIBattleRunner.instance == null) {
            new ai.AIBattleRunner();
        }
        ai.AIBattleRunner.instance.startBattle();
    }

    /**
     * 停止AI自动对战
     */
    public static function stopAIBattle():Void {
        if (ai.AIBattleRunner.instance != null) {
            ai.AIBattleRunner.instance.stopBattle();
        }
    }
     ...
     * 继续AI对战（用户响应询问后调用）
     */
    public static function continueAIBattle():Void {
        if (ai.AIBattleRunner.instance != null) {
            ai.AIBattleRunner.instance.continueBattle();
        }
    }
     ...
     * 获取AI对战历史记录
     */
    public static function getAIBattleHistory():Dynamic {
        if (ai.AIBattleRunner.instance != null) {
            return ai.AIBattleRunner.instance.getBattleHistory();
        }
        return [];
    }
     ...
     * 下载AI对战日志
     */
    public static function downloadAIBattleLog():Void {
        if (ai.AIBattleRunner.instance == null) {
            trace("[AIBattle] 无对战记录");
            return;
        }
        var history = ai.AIBattleRunner.instance.getBattleHistory();
        if (history.length == 0) {
            trace("[AIBattle] 无对战记录");
            return;
        }
        ...
        for (record in history) {
            ...
            lines.push("结果: " + (record.winner != null ? record.winner + "获胜" : "平局"));
            lines.push("回合数: " + record.totalTurns);
            lines.push("");
            lines.push("【游戏日志】");
            for (logLine in record.log) {
                lines.push(logLine);
            }
            if (record.review != null) {
                ...
                for (a in record.review.winnerAnalysis) {
                    lines.push("  - " + a);
                }
                lines.push("败者分析: " + record.review.loser);
                for (a in record.review.loserAnalysis) {
                    lines.push("  - " + a);
                }
                lines.push("关键节点:");
                for (m in record.review.keyMoments) {
                    lines.push("  ◆ " + m);
                }
            }
            lines.push("");
